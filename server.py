...
def find_afterparties():
    ...
    if location and distance and measurement:
        ...
        payload = {'token': token, 'q': query, 'location.address': location, 'location.within': distance, 'sort_by': sort}
...
def create_eventbrite_event():
    ...
    payload = {'event.name.html': name, 'event.start.utc': start_time, 'event.end.utc': end_time, 'event.currency': currency}
